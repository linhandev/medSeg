...
def get_args():
    ...
    parser.add_argument(
        "--size", type=float, default=10.0, help="[必填] 压缩文件过程中每个包不超过这个大小，G为单位"
    )
    parser.add_argument(
        "--del",
        action="store_true",
    ...
    parser.add_argument(
        "--debug", action="store_true", default=False, help="[可选]执行过程中显示详细信息"
    )
...
def do_zip(args):
    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join(args.dataset_dir)):
        for filename in filenames:
            ...
            logging.debug(
                "Name: {}, Size: {}".format(filename, curr_file_size / 1024 ** 2)
            )
            ...
            if list_size >= zip_left_size:  # 如果当前列表中未压缩文件的大小大于zip包能装的大小，那么开始压包
                ...
                if (
                    curr_zip_size + curr_file_size > zip_tot_size
                ):  # 如果加入这个文件压缩包就超大小限制了就开新的压缩包
                    f.close()
                    zip_num += 1
                    curr_name = "{}-{}.zip".format(dataset_name, zip_num)
                    curr_zip_path = os.path.join(args.zip_dir, curr_name)
