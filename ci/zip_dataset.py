...
def get_args():
    ...
    parser.add_argument("--dataset_dir", type=str, required=True, help="[必填] 需要压缩的数据集路径，所有文件所在的文件夹")
    parser.add_argument("--zip_dir", type=str, required=True, help="[必填] 压缩后的压缩包保存路径，如果有条件可以和待压缩数据放到不同硬件上，能加快一点速度")
    parser.add_argument("--size", type=float, default=10.0, help="[必填] 压缩文件过程中每个包不超过这个大小，G为单位")
    parser.add_argument("--del", action="store_true", default=False, help="[慎用]在压缩完文件后删除对应的原文件，如果盘上空间不够压完就删掉原文件不会炸盘。！！慎用此功能！！")
    parser.add_argument("-v", "--verbos", action="store_true", default=False, help="[可选]执行过程中显示详细信息")
...
def do_zip(args):
    ...
    zip_tot_size = args.size * 1024 ** 3  # 每个压缩包不超过这个大小，单位B
    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join(args.dataset_dir)):
        for filename in filenames:
            ...
            if list_size >= zip_left_size:  # 如果当前列表中未压缩文件的大小大于zip包能装的大小，那么开始压包
                ...
                list_size = curr_file_size # 循环中这个pass的文件是在if之后加入列表的，所以列表文件的大小直接就是这个pass文件的大小，下面就添加了
            ...
            files_list.append(
                [
                    ...
                    os.path.join(
                        dataset_name, dirpath[len(args.dataset_dir) + 1 :], filename
                    ),
                ]
            )
