...
volumes = util.listdir(volumes_path)
labels = util.listdir(labels_path)
...
for i in range(len(volumes)):
    ...
    if plt_permutation:
        volume = np.reshape(
            volume, [volume.shape[0] * volume.shape[1] * volume.shape[2]]
        )  # change into 1d array to conduct analasys
        ...
        file = open(
            os.path.join(vol_percentage_path, "{}.txt".format(volumes[i].rstrip(".nii"))), "w"
        )
        ...
        print(
            "[{:<10f},{:<10f}] : {:>10} percentage : {}".format(
                bins[j], bins[j + 1], nums[j], nowsum / sum
            ),
            file=file,
        )
        ...
        file = open(
            os.path.join(lab_percentage_path, "{}.txt".format(volumes[i].rstrip(".nii"))), "w"
        )
        print("--------- {} --------".format(volumes[i]), file=file)
